...
package object barneshut {
  ...
  extends Quad {
    //(bodies.foldLeft(0f)((sum, body) => sum + body.mass))
    // bodies.foldLeft(0f)((sum, body) => sum + body.mass*body.x )/ mass
    //
    val mass = bodies.foldLeft(0f)((sum, body) => sum + body.mass) : Float
    val massX = bodies.foldLeft(0f)((sum, body) => sum + body.mass*body.x )/ mass : Float
    val massY = bodies.foldLeft(0f)((sum, body) => sum + body.mass*body.y )/ mass : Float
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val xValue: Int = (0 to sectorPrecision).indexWhere(i => boundaries.minX + i * sectorSize > b.x) - 1
      val yValue: Int = (0 to sectorPrecision).indexWhere(i => boundaries.minY + i * sectorSize > b.y) - 1
      matrix(xValue + yValue * sectorPrecision) = matrix(xValue + yValue * sectorPrecision ) += b
      this
    }
