...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)((counter, article) => if (article.text.split(" ").contains(lang)) counter + 1 else counter , _ + _)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(art => langs.filter(l => art.text.split(" ").contains(l)).map(l => (l, 1))).reduceByKey(_ + _).collect.toList.sortBy(_._2)(Ordering[Int].reverse)
